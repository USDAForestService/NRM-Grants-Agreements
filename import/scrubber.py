...
def anonymize_json(source, target=None):
    ...
    try:
        replacements_file = open("replacements.txt", "r")
        print("Opening existing REPLACEMENTS file")
        REPLACEMENTS = json.loads(replacements_file.read())
        replacements_file.close()
    except IOError:
        print("Replacements does not yet exist. We'll create it.")
        REPLACEMENTS = {}
